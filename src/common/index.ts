...
/**
 * @param Store: Main class to set items to storage or settings and allow them to be cleared by button in PronounDB afterwards.
 * 
 * @param item: Either sets a Setting or stores an item in Storage depending on the object passed, and also adds it to a list of state to be cleared from @arg {</PronounDB:1> type:Clear Stores} and from the @arg settings panel
 */
...
/**
 * @param PronounMap: The main Record of all the different Pronoun types and their mapping to Plain text.
 */
import { ProunounMap } from './map';
